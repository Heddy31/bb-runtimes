--
--  Copyright (C) 2021, AdaCore
--

pragma Style_Checks (Off);

--  This spec has been automatically generated from STM32L4x2.svd


with System;

package Interfaces.STM32.COMP is
   pragma Preelaborate;
   pragma No_Elaboration_Code_All;

   ---------------
   -- Registers --
   ---------------

   subtype COMP1_CSR_COMP1_EN_Field is Interfaces.STM32.Bit;
   subtype COMP1_CSR_COMP1_PWRMODE_Field is Interfaces.STM32.UInt2;
   subtype COMP1_CSR_COMP1_INMSEL_Field is Interfaces.STM32.UInt3;
   subtype COMP1_CSR_COMP1_INPSEL_Field is Interfaces.STM32.UInt2;
   subtype COMP1_CSR_COMP1_POLARITY_Field is Interfaces.STM32.Bit;
   subtype COMP1_CSR_COMP1_HYST_Field is Interfaces.STM32.UInt2;
   subtype COMP1_CSR_COMP1_BLANKING_Field is Interfaces.STM32.UInt3;
   subtype COMP1_CSR_COMP1_BRGEN_Field is Interfaces.STM32.Bit;
   subtype COMP1_CSR_COMP1_SCALEN_Field is Interfaces.STM32.Bit;
   subtype COMP1_CSR_COMP1_INMESEL_Field is Interfaces.STM32.UInt2;
   subtype COMP1_CSR_COMP1_VALUE_Field is Interfaces.STM32.Bit;
   subtype COMP1_CSR_COMP1_LOCK_Field is Interfaces.STM32.Bit;

   --  Comparator 1 control and status register
   type COMP1_CSR_Register is record
      --  Comparator 1 enable bit
      COMP1_EN       : COMP1_CSR_COMP1_EN_Field := 16#0#;
      --  unspecified
      Reserved_1_1   : Interfaces.STM32.Bit := 16#0#;
      --  Power Mode of the comparator 1
      COMP1_PWRMODE  : COMP1_CSR_COMP1_PWRMODE_Field := 16#0#;
      --  Comparator 1 Input Minus connection configuration bit
      COMP1_INMSEL   : COMP1_CSR_COMP1_INMSEL_Field := 16#0#;
      --  Comparator1 input plus selection bit
      COMP1_INPSEL   : COMP1_CSR_COMP1_INPSEL_Field := 16#0#;
      --  unspecified
      Reserved_9_14  : Interfaces.STM32.UInt6 := 16#0#;
      --  Comparator 1 polarity selection bit
      COMP1_POLARITY : COMP1_CSR_COMP1_POLARITY_Field := 16#0#;
      --  Comparator 1 hysteresis selection bits
      COMP1_HYST     : COMP1_CSR_COMP1_HYST_Field := 16#0#;
      --  Comparator 1 blanking source selection bits
      COMP1_BLANKING : COMP1_CSR_COMP1_BLANKING_Field := 16#0#;
      --  unspecified
      Reserved_21_21 : Interfaces.STM32.Bit := 16#0#;
      --  Scaler bridge enable
      COMP1_BRGEN    : COMP1_CSR_COMP1_BRGEN_Field := 16#0#;
      --  Voltage scaler enable bit
      COMP1_SCALEN   : COMP1_CSR_COMP1_SCALEN_Field := 16#0#;
      --  unspecified
      Reserved_24_24 : Interfaces.STM32.Bit := 16#0#;
      --  comparator 1 input minus extended selection bits
      COMP1_INMESEL  : COMP1_CSR_COMP1_INMESEL_Field := 16#0#;
      --  unspecified
      Reserved_27_29 : Interfaces.STM32.UInt3 := 16#0#;
      --  Read-only. Comparator 1 output status bit
      COMP1_VALUE    : COMP1_CSR_COMP1_VALUE_Field := 16#0#;
      --  Write-only. COMP1_CSR register lock bit
      COMP1_LOCK     : COMP1_CSR_COMP1_LOCK_Field := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;

   for COMP1_CSR_Register use record
      COMP1_EN       at 0 range 0 .. 0;
      Reserved_1_1   at 0 range 1 .. 1;
      COMP1_PWRMODE  at 0 range 2 .. 3;
      COMP1_INMSEL   at 0 range 4 .. 6;
      COMP1_INPSEL   at 0 range 7 .. 8;
      Reserved_9_14  at 0 range 9 .. 14;
      COMP1_POLARITY at 0 range 15 .. 15;
      COMP1_HYST     at 0 range 16 .. 17;
      COMP1_BLANKING at 0 range 18 .. 20;
      Reserved_21_21 at 0 range 21 .. 21;
      COMP1_BRGEN    at 0 range 22 .. 22;
      COMP1_SCALEN   at 0 range 23 .. 23;
      Reserved_24_24 at 0 range 24 .. 24;
      COMP1_INMESEL  at 0 range 25 .. 26;
      Reserved_27_29 at 0 range 27 .. 29;
      COMP1_VALUE    at 0 range 30 .. 30;
      COMP1_LOCK     at 0 range 31 .. 31;
   end record;

   subtype COMP2_CSR_COMP2_EN_Field is Interfaces.STM32.Bit;
   subtype COMP2_CSR_COMP2_PWRMODE_Field is Interfaces.STM32.UInt2;
   subtype COMP2_CSR_COMP2_INMSEL_Field is Interfaces.STM32.UInt3;
   subtype COMP2_CSR_COMP2_INPSEL_Field is Interfaces.STM32.UInt2;
   subtype COMP2_CSR_COMP2_WINMODE_Field is Interfaces.STM32.Bit;
   subtype COMP2_CSR_COMP2_POLARITY_Field is Interfaces.STM32.Bit;
   subtype COMP2_CSR_COMP2_HYST_Field is Interfaces.STM32.UInt2;
   subtype COMP2_CSR_COMP2_BLANKING_Field is Interfaces.STM32.UInt3;
   subtype COMP2_CSR_COMP2_BRGEN_Field is Interfaces.STM32.Bit;
   subtype COMP2_CSR_COMP2_SCALEN_Field is Interfaces.STM32.Bit;
   subtype COMP2_CSR_COMP2_INMESEL_Field is Interfaces.STM32.UInt2;
   subtype COMP2_CSR_COMP2_VALUE_Field is Interfaces.STM32.Bit;
   subtype COMP2_CSR_COMP2_LOCK_Field is Interfaces.STM32.Bit;

   --  Comparator 2 control and status register
   type COMP2_CSR_Register is record
      --  Comparator 2 enable bit
      COMP2_EN       : COMP2_CSR_COMP2_EN_Field := 16#0#;
      --  unspecified
      Reserved_1_1   : Interfaces.STM32.Bit := 16#0#;
      --  Power Mode of the comparator 2
      COMP2_PWRMODE  : COMP2_CSR_COMP2_PWRMODE_Field := 16#0#;
      --  Comparator 2 Input Minus connection configuration bit
      COMP2_INMSEL   : COMP2_CSR_COMP2_INMSEL_Field := 16#0#;
      --  Comparator 2 Input Plus connection configuration bit
      COMP2_INPSEL   : COMP2_CSR_COMP2_INPSEL_Field := 16#0#;
      --  Windows mode selection bit
      COMP2_WINMODE  : COMP2_CSR_COMP2_WINMODE_Field := 16#0#;
      --  unspecified
      Reserved_10_14 : Interfaces.STM32.UInt5 := 16#0#;
      --  Comparator 2 polarity selection bit
      COMP2_POLARITY : COMP2_CSR_COMP2_POLARITY_Field := 16#0#;
      --  Comparator 2 hysteresis selection bits
      COMP2_HYST     : COMP2_CSR_COMP2_HYST_Field := 16#0#;
      --  Comparator 2 blanking source selection bits
      COMP2_BLANKING : COMP2_CSR_COMP2_BLANKING_Field := 16#0#;
      --  unspecified
      Reserved_21_21 : Interfaces.STM32.Bit := 16#0#;
      --  Scaler bridge enable
      COMP2_BRGEN    : COMP2_CSR_COMP2_BRGEN_Field := 16#0#;
      --  Voltage scaler enable bit
      COMP2_SCALEN   : COMP2_CSR_COMP2_SCALEN_Field := 16#0#;
      --  unspecified
      Reserved_24_24 : Interfaces.STM32.Bit := 16#0#;
      --  comparator 2 input minus extended selection bits
      COMP2_INMESEL  : COMP2_CSR_COMP2_INMESEL_Field := 16#0#;
      --  unspecified
      Reserved_27_29 : Interfaces.STM32.UInt3 := 16#0#;
      --  Read-only. Comparator 2 output status bit
      COMP2_VALUE    : COMP2_CSR_COMP2_VALUE_Field := 16#0#;
      --  Write-only. COMP2_CSR register lock bit
      COMP2_LOCK     : COMP2_CSR_COMP2_LOCK_Field := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;

   for COMP2_CSR_Register use record
      COMP2_EN       at 0 range 0 .. 0;
      Reserved_1_1   at 0 range 1 .. 1;
      COMP2_PWRMODE  at 0 range 2 .. 3;
      COMP2_INMSEL   at 0 range 4 .. 6;
      COMP2_INPSEL   at 0 range 7 .. 8;
      COMP2_WINMODE  at 0 range 9 .. 9;
      Reserved_10_14 at 0 range 10 .. 14;
      COMP2_POLARITY at 0 range 15 .. 15;
      COMP2_HYST     at 0 range 16 .. 17;
      COMP2_BLANKING at 0 range 18 .. 20;
      Reserved_21_21 at 0 range 21 .. 21;
      COMP2_BRGEN    at 0 range 22 .. 22;
      COMP2_SCALEN   at 0 range 23 .. 23;
      Reserved_24_24 at 0 range 24 .. 24;
      COMP2_INMESEL  at 0 range 25 .. 26;
      Reserved_27_29 at 0 range 27 .. 29;
      COMP2_VALUE    at 0 range 30 .. 30;
      COMP2_LOCK     at 0 range 31 .. 31;
   end record;

   -----------------
   -- Peripherals --
   -----------------

   --  Comparator
   type COMP_Peripheral is record
      --  Comparator 1 control and status register
      COMP1_CSR : aliased COMP1_CSR_Register;
      --  Comparator 2 control and status register
      COMP2_CSR : aliased COMP2_CSR_Register;
   end record
     with Volatile;

   for COMP_Peripheral use record
      COMP1_CSR at 16#0# range 0 .. 31;
      COMP2_CSR at 16#4# range 0 .. 31;
   end record;

   --  Comparator
   COMP_Periph : aliased COMP_Peripheral
     with Import, Address => COMP_Base;

end Interfaces.STM32.COMP;
